...
def inverseByAdjointMatrix(A):  # 수반행렬을 이용한 A의 역행렬 계산
    ...
    for i in range(0, n):  # 수반행렬 생성
        for j in range(0, m):
            adjA[j, i] = cofactor(A, i, j)  # adjA의 (j, i) 위치에 A의 (i, j) 원소에 대한 여인수(cofactor)를 할당(전치)

    if detA != 0.0:  # A의 행렬식이 0이 아닌 경우
        # [수업자료 5.행렬식] 정리 5-32) 수반행렬 adjA를 이용한 역행렬 계산
